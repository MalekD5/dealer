//! A minimal SHA-512 implementation (FIPS 180-4).
...
impl Sha512 {
    ...
    pub fn finish(mut self) -> [u8; 64] {
        ...
        // The padding is a single `1` bit, zeroes, then the 128-bit big-endian
        // message length; together they must land on a block boundary.
        self.update_without_counting(&[0x80]);
        while self.buffered != BLOCK_LENGTH - 16 {
            self.update_without_counting(&[0x00]);
        }
        self.update_without_counting(&bit_length.to_be_bytes());

        let mut digest = [0; 64];
        for (target, word) in digest.chunks_exact_mut(8).zip(self.state) {
    ...
    fn compress(&mut self, block: &[u8]) {
        ...
        for (word, source) in schedule.iter_mut().zip(block.chunks_exact(8)) {
            *word = u64::from_be_bytes(source.try_into().expect("chunk is eight bytes"));
        }
        for index in 16..80 {
            ...
            let sigma1 = recent.rotate_right(19) ^ recent.rotate_right(61) ^ (recent >> 6);
        ...
        for (current, added) in self.state.iter_mut().zip([a, b, c, d, e, f, g, h]) {
